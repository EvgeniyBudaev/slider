(function() {
  ...
  const Gallery = function(id, setup) {
...
function extend(out) {
	out = out || {};
	// перебираем в цикле аргументы функции
	for (var i = 1; i < arguments.length; i++) {
		if (!arguments[i])
			continue;
		// перебираем свойства объекта переданного в аргументе и
		// записываем имя свойства и его значение в новый объект
...
fn.setSizeCarousel = function() {
  ...
  this.widthSlider = this.slider.offsetWidth;
  	...
  	// если разрешена адаптивность галереи, то необходимо, используя свойство
	...
	// получаем ширину элемента слайдера, которая зависит от количества
	...
	// другими словами - расстоянием между левыми границами элементов слайдера
...
fn.initControl = function() {
	// объект с кнопками навигации 'prev' и 'next'
...
fn.creatDotsCtrl = function() {
	...
	// устанавливаем стили элементам постраничной навигации
	this.setDotsStyle();
};
...
fn.getNextCoordinates = function(direction) {
	...
	// галерея при автоскролле прокручена до конца, возвращаемся к первому элементу
	if (this.options.autoScroll && this.current >= this.count - this.options.visibleItems) {
		...
		if (this.current == 0 && direction == -1 || 
			// попытка прокрутить к следующему элементу, когда текущим является элемент
			// с индексом max
...
fn.dotsControl = function(e) {
	...
	// находим индекс элемента 'span' в массиве 'spots', этот индекс
	// понадобится для поиска координаты в массиве 'coordinates'
...
fn.keyControl = function(e) {
	...
	// если код не соответствует клавишам 'влево' или 'вправо',
	// прекращаем работу функции
...
fn.drag = function(e) {
	...
	var delta = this.coordinates[this.current] - this.shift;
...
fn.resize = function() {
	...
	this.resizeTimer = setTimeout(function() {
		...
		// прокручиваем слайдер до текущего элемента
		this.scroll.apply(this, [x, this.options.baseTransition]);
